...
export class GeminiProvider implements AiProvider {
  ...
  async execute(request: AiProviderRequest): Promise<AiProviderRawResponse> {
    ...
    if (!text) {
      throw new AiExecutionError({
        ...
        message: "Gemini returned an empty response for this analysis stage.",
        status: 502,
        stageId: request.stageId,
      });
    }
...
function mapGeminiError(error: unknown, request: AiProviderRequest): AiExecutionError {
  ...
  if (error instanceof GoogleGenerativeAIAbortError) {
    return new AiExecutionError({
      ...
      status: 504,
      stageId: request.stageId,
      details: { promptEstimatedBytes: request.prompt.estimatedBytes },
    });
  }
  ...
  if (error instanceof GoogleGenerativeAIFetchError) {
    ...
    return new AiExecutionError({
      code: status === 429 ? "AI_PROVIDER_RATE_LIMITED" : "AI_PROVIDER_ERROR",
      message:
        status === 429
